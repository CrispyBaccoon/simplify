...
function applySettings(settings) {
	...
	*/
    ...
    if (simplifyDebug)
        console.log("Apply settings: " + JSON.stringify(settings));
}
    ...
    if (
        ...
        if (simplSettings.kbsMenu) {
            ...
            if (!document.querySelector(".aeN").classList.contains("bhZ")) {
                document
                    .querySelector('div[role="navigation"] a:first-child')
                    .focus();
            }
...
function detectClassNames() {
    ...
    if (searchForm) {
        ...
        if (detectClassNamesLoops < 10) {
            ...
            if (simplifyDebug)
                console.log("Giving up on detecting class names");
        }
    }
}
...
function initStyle() {
    if (document.head) {
        ...
        addCSS(
            `:root { --readingPane-width: ${simplify[u].readingPaneWidth}; }`
        );
        if (simplifyDebug)
        ...
        addCSS(
            `:root { --add-on-height: ${
                simplify[u].addOnsCount * 56
            }px !important; }`
        );
        if (simplifyDebug)
            console.log(
                "Just made room for " + simplify[u].addOnsCount + " add ons"
            );
    ...
    if (searchForm) {
        ...
        searchIcon.addEventListener(
            ...
            function (event) {
                ...
                updateParam(
                    "minimizeSearch",
                    htmlEl.classList.contains("hideSearch")
                );
                toggleSearchFocus();
            },
            false
        ...
        searchCloseIcon.addEventListener(
            ...
            function (event) {
                ...
                if (location.hash == closeSearchUrl) {
                    ...
                    const searchFormClass = searchForm.classList.value.split(
                        " "
                    )[2];
    ...
    if (searchInput) {
        ...
        searchInput.addEventListener("blur", (e) => {
            ...
            if (
                ...
                if (simplifyDebug)
                    console.log("Hide search. Clicked on", e.target);
                htmlEl.classList.add("hideSearch");
            } else {
                if (simplifyDebug)
...
function detectTheme() {
    const msgCheckbox = document.querySelectorAll(
        'div[gh="tl"] .xY > .T-Jo'
    )[0];
...
function detectDensity() {
    ...
    if (navItem) {
        ...
        detectDensityLoops++;
        if (simplifyDebug)
            console.log("detectDensity loop #" + detectDensityLoops);
...
function detectReadingPane() {
    ...
    if (inMsg) {
        ...
        if (simplifyDebug)
            console.log(
                "Simplify: in conversation view: detect Reading Pane later"
            );
        return;
    } else {
    ...
    if (offlineActive && detectReadingPaneLoops == 0) {
        ...
        const readingPaneMenu = document.querySelector(
            'div[selector="nosplit"]'
        );
        if (readingPaneMenu) {
            ...
            if (noSplitToggle) {
                if (noSplitToggle.length == 0) {
                    if (simplifyDebug)
                        console.log("Reading pane detected and active");
            ...
            const readingPaneObserverCallback = function (
                mutationsList,
                observer
            ) {
                ...
                for (let mutation of mutationsList) {
                    if (
                        ...
                        if (simplifyDebug)
                            console.log(
                                "Reading pane mode toggled. Detecting new state..."
                            );
                        if (
                            mutation.target.attributes.class.value.indexOf(
                                "apJ"
                            ) > -1
                        ) {
                            htmlEl.classList.remove("noSplitPane");
            ...
            if (simplifyDebug)
                console.log(
                    "Detect preview pane loop #" + detectReadingPaneLoops
                );
            ...
            if (detectReadingPaneLoops < 10) {
                ...
                if (simplifyDebug)
                    console.log("Giving up on detecting reading pane");
    ...
    if (simplify[u].noSplitPane) {
        ...
        if (leftPane) {
            ...
            if (leftPaneWidth == "auto") {
                ...
                if (simplifyDebug)
                    console.log(
                        "Detect preview pane width loop #" +
                            detectReadingPaneWidthLoops
                    );
                if (detectReadingPaneWidthLoops < 10) {
                    ...
                    if (simplifyDebug)
                        console.log(
                            "Giving up on detecting reading pane width"
                        );
                }
            } else {
...
function detectNumberOfAddOns() {
    ...
    if (numberOfAddOns > 0) {
        ...
        if (simplifyDebug)
            console.log(
                "detectNumberOfAddOns loop #" + detectNumberOfAddOnsLoops
            );
...
function detectAddOns() {
    ...
    if (addOnsPane) {
        ...
        const addOnsObserverCallback = function (mutationsList, observer) {
            for (let mutation of mutationsList) {
                if (
                    ...
                    if (
                        mutation.target.attributes.class.value.indexOf("br3") >
                        -1
                    ) {
                        htmlEl.classList.remove("addOnsOpen");
                        updateParam("addOns", false);
        ...
        // Start observing the target node for configured mutations
        if (simplifyDebug)
            console.log("Adding mutation observer for Add-ons Pane");
        ...
        if (detectAddOnsPaneLoops < 10) {
            ...
            if (simplifyDebug)
                console.log("Giving up on detecting add-ons pane");
        }
    }
}
...
function detectRightSideChat() {
    ...
    if (talkRoster) {
            ...
            const rosterObserver = new MutationObserver(
                (mutationsList, observer) => {
                    mutationsList.forEach((mutation) => {
                        if (
                            mutation.target.attributes.style.value.search(
                                "height: 0px"
                            ) === -1
                        ) {
                            // Roster open
                            updateParam("chatClosed", false);
                            if (simplifyDebug)
                                console.log("Chat roster opened");
                        } else {
                            // Roster minimized
                            updateParam("chatClosed", true);
                            if (simplifyDebug)
                                console.log("Chat roster closed");
                        }
                    });
                }
            );
        ...
        if (detectRightSideChatLoops < 10) {
            ...
            if (simplifyDebug)
                console.log("Giving up on detecting Talk roster");
        }
    }
}
...
function detectButtonLabel() {
    ...
    if (secondButton) {
        ...
        if (detectButtonLabelLoops < 5) {
            ...
            if (simplifyDebug)
                console.log(
                    "Detect button labels loop #" + detectButtonLabelLoops
                );
        }
    }
}
...
function toggleMenu() {
    ...
    if (simplify[u].navOpen) {
        ...
        if (simplifyDebug)
            console.log(
                "Toggle nav to hidden; simplify.navOpen = " +
                    simplify[u].navOpen
            );
    } else {
        htmlEl.classList.add("navOpen");
        ...
        if (simplifyDebug)
            console.log(
                "Toggle nav to shown;  simplify.navOpen = " +
                    simplify[u].navOpen
            );
    }
}
    ...
    if (!inInbox) {
        ...
        if (inboxLoaded && threadList) {
            ...
            if (multiBoxes.length > 1) {
                ...
                let inboxWidth = parseInt(
                    window.getComputedStyle(threadList).width
                );
                if (simplifyDebug)
                    console.log(
                ...
                if (inboxWidth <= sectionWidth * 2) {
                    if (simplifyDebug)
                        console.log("Multiple inboxes are vertical");
                    ...
                    if (simplifyDebug)
                        console.log("Multiple inboxes are side by side");
                    htmlEl.classList.add("multiBoxHorz");
    ...
    if (actionBar) {
        ...
        paginationDivs.forEach(function (pagination) {
            ...
            if (msgCountDivs.length == 3) {
                ...
                if (simplifyDebug)
                    console.log("Didn't find pagination controls");
            }
    ...
    if (otherExtensions.length > 0) {
        ...
        Object.entries(otherExtensionsList).forEach(function (extension) {
            ...
            if (extensionEl) {
                ...
                if (simplifyDebug)
                    console.log(
                        `Extensions - right position now: ${extensionsWidth}px`
                    );
            } else {
                if (simplifyDebug)
...
function observeThreadlist() {
    ...
    if (threadlist) {
        if (simplSettings.dateGrouping) {
            ...
            if (simplifyDebug)
                console.log("Adding mutation observer for threadlist");
        }
    } else {
        if (observeThreadlistLoops < 10) {
            ...
            if (simplifyDebug)
                console.log(
                    "observeThreadlist attempt #" + observeThreadlistLoops
                );
        }
    }
}
